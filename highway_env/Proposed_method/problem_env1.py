...
class MyHighwayEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #         target_iteration = self.iteration - 1  # 上一个偶数
        #         if target_iteration in self.reward_house:
        #             current_rewards = self.reward_house[target_iteration]
        ...
        #                     line = f.read().strip()
        #                     match = re.match(r"Iteration (\d+): (\[.*\])", line)
        #                     if match:
        #                         old_rewards = ast.literal_eval(match.group(2))
        #                         old_mean = np.mean(old_rewards)
        #                     else:
        #                         old_mean = -float("inf")  # 若格式不匹配，强制当前写入
        #             except FileNotFoundError:
        #                 old_mean = -float("inf")  # 文件不存在，直接写
        #             except Exception as e:
        #                 print(f"[Warning] Failed to read reward.txt: {e}")
        #                 old_mean = -float("inf")
        #
        #             if current_mean > old_mean:
        #                 print(f"current_mean: {current_mean}")
        #                 print(f"old_mean: {old_mean}")
        #                 with open("reward.txt", "w") as f:
        #                     f.write(f"Iteration {target_iteration}: {current_rewards}\n")
        #                     f.flush()
